...
const removeUser = (socketId) => {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    userLogout = users[i].socketDevice = users[i].socketDevice.filter(
      (socketDevice) => socketDevice != socketId
    );
...
io.on("connection", (socket) => {
  ...
  socket.on("addUser", (data) => {
    try {
      ...
      if (!users.some((user) => user.id === data.userId)) {
  ...
  socket.on("missCall", (data) => {
    try {
      let usercaller = getUser(data.caller);
      usersCalling = usersCalling.filter(
        (room) => !Object.keys(room).some((user) => user == data.caller)
      );
      console.log(
        "Roome Calling Event Miss Call.....................",
        usersCalling
      );
        ...
        if (isCalling) {
          let room = usersCalling.find((room) =>
            Object.keys(room).some(
              (userCalling) =>
                userCalling == user.id &&
                room[userCalling] == user.socketDisconnect
            )
          );
          usersCalling = usersCalling.filter(
            (room) =>
